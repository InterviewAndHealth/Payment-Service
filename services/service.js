...
class Service {
  ...
  async createCheckoutSession(
    ...
    if (promocode) {
      discount = await this.applyPromocode({promocode, user_id, role, currency: product.currency});
      ...
      if (isNaN(finalPrice) || finalPrice < 0) {
        throw new BadRequestError(
          "Invalid final price after applying discount"
        );
      }
    }
  ...
  async webhookservice(sig, info) {
    ...
    if (event.type === "invoice.paid") {
      ...
      const stripe_subscription_id =
        invoice?.subscription || invoice?.discount?.subscription;
      await this.repository.updateSubscriptionStatus(
        stripe_subscription_id,
        "ACTIVE"
      );
      const stripe_customer_id =
        invoice?.customer || invoice?.discount?.customer;
  ...
  async getAllPromocodes(){
    return await this.repository.getAllPromocodes()
  }

  async applyPromocode({promocode, user_id, role, currency}) {
    const promocodeExists = await this.repository.checkPromoCodeExists(
      promocode,
      role
    );
    ...
    if (promocodeExists.currency !== "ALL") {
      if (promocodeExists.currency !== currency) {
        throw new BadRequestError("Promocode does not exist.");
      }
    }
    ...
    if (referral_code) {
      ...
      if (referrer) {
        ...
        if (referral.total_referrals % 3 === 0) {
          ...
          await this.repository.assignPromoCode(
            promo_code.id,
            referrer.user_id
          );
        }
